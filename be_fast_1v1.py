def play_be_fast_1v1():
    ...
    def loose(x, j):
        ...
        display.set_pen(white); display.text("Game over", 20, 100, scale=4); display.update(); sleep(2)
        display.set_pen(bg); display.rectangle(int(0), int(0), int(240), int(240)); display.update()
        sleep(0.01)
        if x<3 and j<3:
            display.set_pen(white); display.text("0-1", 5, 50, scale=19); display.update(); sleep(0.1)
        elif x>2 and j>2:
            display.set_pen(white); display.text("1-0", 5, 50, scale=19); display.update(); sleep(0.1)
        elif x<3 and j>2:
            display.set_pen(white); display.text("1-0", 5, 50, scale=19); display.update(); sleep(0.1)
        elif x>2 and j<3:
            display.set_pen(white); display.text("0-1", 5, 50, scale=19); display.update(); sleep(0.1)
    ...
    while ok:  
        if x[i] == 1:
            display.set_pen(white); display.rectangle(0, 0, 120, 120)
        elif x[i] == 2:
            display.set_pen(white); display.rectangle(0, 120, 120, 120)
        elif x[i] == 3:
            display.set_pen(white); display.rectangle(120, 0, 120, 120)
        elif x[i] == 4:
            display.set_pen(white); display.rectangle(120, 120, 120, 120)
        display.update()
        last_time = ticks_ms()
        while True:
            if A.value():
            ...
            if Y.value():
                ...
                loose(x[i], 4); ok=0; break
            if ticks_ms() - last_time > j:
                loose(x[i], x[i]); ok=0; break
        if ok:
            pwm.duty_u16(0); display.set_pen(bg); display.rectangle(int(0), int(0), int(240), int(240)); display.update()
