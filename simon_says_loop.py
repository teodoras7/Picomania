def play_simon_says_loop():
    ...
    def loose(j):
        ...
        sleep(0.01)
        if j<11:
            display.set_pen(white); display.text("Score " + str(j-1), 20, 100, scale=6); display.update()
        else:
            display.set_pen(white); display.text("Score " + str(j-1), 20, 100, scale=5); display.update()
